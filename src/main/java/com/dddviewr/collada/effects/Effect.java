...
public class Effect extends Base {
  ...
  public void dump(PrintStream out, int indent) {
    ...
    if (this.effectMaterial != null) this.effectMaterial.dump(out, indent + 1);
  }
  ...
  public NewParam findNewParam(String sid) {
    if (this.newParams == null) return null;
    String search = sid;
    if (sid.indexOf(35) == 0) search = sid.substring(1);
    for (NewParam p : this.newParams) {
      if (search.equals(p.getSid())) return p;
    }
    return null;
  }
}
