...
public class StateManager extends DefaultHandler {
  ...
  public void characters(char[] ch, int start, int length) throws SAXException {
    if (!(this.states.isEmpty())) ((State) this.states.peek()).characters(ch, start, length);
  }

  public void endElement(String uri, String localName, String qName) throws SAXException {
    if (localName == null || localName.length() == 0) localName = qName;
  ...
  public void startElement(String uri, String localName, String qName, Attributes attributes)
    ...
    if (this.states.isEmpty()) {
      ...
      if (current instanceof COLLADA) this.collada = ((COLLADA) current).getCollada();
    } else {
  ...
  public State getParent(State state) {
    ...
    if (idx > 0) return ((State) this.states.get(idx - 1));
    return result;
  }
