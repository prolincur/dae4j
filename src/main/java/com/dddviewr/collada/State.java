...
public class State {
  ...
  public void characters(char[] ch, int start, int length) {
    if (!(this.contentNeeded)) return;

    int newStart = start;
    int newLength = length;
    boolean foundPreSpace = false;
    while (newStart < start + length) {
      if (!(Character.isWhitespace(ch[newStart]))) break;
      foundPreSpace = true;
      ++newStart;
      --newLength;
    }
    ...
    while (newLength > 0) {
      if (!(Character.isWhitespace(ch[pos]))) break;
      --pos;
      --newLength;
    }

    if (newLength > 0) {
      if ((!(firstChars)) && (foundPreSpace)) content.append(' ');
