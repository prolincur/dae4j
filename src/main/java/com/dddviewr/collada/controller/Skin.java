...
public class Skin extends Base {
  ...
  public void dump(PrintStream out, int indent) {
    ...
    if (this.bindShapeMatrix != null)
      out.println(prefix + " BindShapeMatrix: " + Arrays.toString(this.bindShapeMatrix));
    for (Source src : this.sources) {
      src.dump(out, indent + 1);
    }
    if (this.joints != null) this.joints.dump(out, indent + 1);
    if (this.vertexWeights != null) this.vertexWeights.dump(out, indent + 1);
  }

  public void deindex(int[] newVertList) {
    int i;
    int l;
    if (this.vertexWeights == null) return;
    ...
    for (Input inp : this.vertexWeights.getInputs()) {
      if (inp.getOffset() <= vsize) continue;
      vsize = inp.getOffset();
    }
    ++vsize;
  ...
  public Source getSource(String id) {
    String searchId = id;
    if (id.startsWith("#")) searchId = id.substring(1);
    for (Source s : this.sources) {
      if (s.getId().equals(searchId)) return s;
    }
    return null;
  }
  ...
  public String[] getJointData() {
    ...
    if (src.getIdrefArray() != null) return src.getIdrefArray().getData();
    if (src.getNameArray() != null) return src.getNameArray().getData();
    return null;
  }
  ...
  public void parseBindShapeMatrix(StringBuilder str) {
    ...
    for (String s : values) {
      ...
      if (s.length() != 0) data[(index++)] = Float.parseFloat(s);
    }
