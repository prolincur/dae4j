...
public class LibraryControllers extends Base {
  ...
  public void dump(PrintStream out, int indent) {
    ...
    for (Controller c : this.controllers) c.dump(out, indent + 1);
  }
  ...
  public List<Skin> findSkins(String source) {
    ...
    for (Controller c : this.controllers) {
      if (c.getSkin() == null) continue;
      String skinSrc = c.getSkin().getSource();
      if (skinSrc == null) continue;
      String tmpSrc = (skinSrc.indexOf(35) == 0) ? skinSrc.substring(1) : skinSrc;
      if (!(tmpSrc.equals(source))) continue;
