...
public class polylist extends State {
  ...
  public void endElement(String name) {
    if ("polylist".equals(name)) {
      ...
      for (Input inp : inputs) thePolyList.addInput(inp);
  ...
  public void addData(StringBuilder str) {
    if (data == null) {
      int count = 0;
      for (int c : theVcount.getData()) count += c;
    ...
    for (String s : vals) {
      ...
      if (s.length() != 0) data[(pos++)] = Integer.parseInt(s);
    }
  }
  ...
  public int getStride() {
    int maxOffset = -1;
    for (Input i : inputs) {
      if (i.getOffset() > maxOffset) maxOffset = i.getOffset();
    }
    return (maxOffset + 1);
  }
}
